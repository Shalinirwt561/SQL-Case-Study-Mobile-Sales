...
--FETCH DATA FROM TABLE THE FAC_TRANSACTIONS 
SELECT * FROM FACT_TRANSACTIONS

--FETCH DATA FROM TABLE THE  DIM_CUSTOMER 
SELECT * FROM DIM_CUSTOMER

--FETCH DATA FROM THE TABLE DIM_DATE 
SELECT * FROM DIM_DATE

--FETCH DATA FROM THE TABLE DIM_LOCATION
SELECT * FROM DIM_LOCATION

--FETCH DATA FROM THE DIM_MANUFACTURE
SELECT * FROM DIM_MANUFACTURER

--FETCH DATA FROM THE TABLE DIM_MODEL
...
SELECT  T1.State,SUM(QUANTITY) AS CNT ,YEAR(T2.DATE) AS CELLPHONES_FROM_2005_TILL_TODAY
FROM DIM_LOCATION AS T1
JOIN FACT_TRANSACTIONS AS T2
ON T1.IDLocation = T2.IDLocation 
WHERE YEAR(T2.DATE)>=2005 
GROUP BY T1.State,T2.Date ) AS A
...
--Q3) SHOW THE NUMBER OF TRNASACTIONS FOR EACH MODEL PER ZIP CODE PER STATE .

SELECT T1.IDMODEL , T2.STATE , T2.ZIPCODE , COUNT(*) AS CNT 
...
SELECT TOP 1 
M.MODEL_NAME  , UNIT_PRICE AS MIN_PRICE 
FROM DIM_MODEL AS M 
...
SELECT  T1.IDMODEL , AVG(TOTALPRICE) AS AVG_PRICE  ,SUM(QUANTITY)  AS TOTAL_QUANTITY
...
--Q6) LIST THE NAME OF THE CUSTOMERS AND THE AVERAGE AMOUNT SPENT IN 2009 , WHERE THE AVERAGE IS HIGHER THAN 500.

SELECT  T2.Customer_Name , AVG(TOTALPRICE) AS AVG_PRICE 
FROM FACT_TRANSACTIONS AS T1
...
SELECT * FROM (
SELECT  TOP 1 * FROM (
SELECT TOP 2  T3.Manufacturer_Name  , YEAR(DATE) AS YEAR_ , SUM(TOTALPRICE) AS SALES
FROM FACT_TRANSACTIONS AS T1
JOIN DIM_MODEL AS T2
ON T1.IDModel = T2.IDModel
JOIN DIM_MANUFACTURER AS T3 
...
--Q10) FIND TOP 100 CUSTOMERS AND THEIR AVERAGE SPEND , AVERAGE QUANTITY BY EACH YEAR . ALSO FIND THE PERCENTAGE OF CHANGE IN THIR SPEND .

SELECT * , ((AVG_ - LAG_PRICE )/LAG_PRICE) AS PERCENTAGE_CHANGE FROM (
SELECT * , LAG(AVG_ , 1) OVER(PARTITION BY IDCUSTOMER ORDER BY YEAR_) AS LAG_PRICE FROM(

SELECT IDCustomer , YEAR(DATE) AS YEAR_ , AVG(TOTALPRICE) AS AVG_ , SUM(QUANTITY) AS QTY
FROM FACT_TRANSACTIONS 
WHERE IDCustomer IN (
		SELECT  TOP 10  T1.IDCustomer 
		FROM FACT_TRANSACTIONS AS T1 
